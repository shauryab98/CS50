...
// global array to remember placemarks
var placemarks = [];

// global array to remember markers
var markers = [];

// global number of seats available
var seatsAvailable = SEATS;

// global total number of passengers left
var passengersLeft = PASSENGERS.length;
...
$(window).load(function() {
    ...
    // listen for speed increase button
    $("#speedup").click(function(event) {
        changeVelocity(10);
    });

    // listen for speed decrease button
...
* Changes velocity.
*/
function changeVelocity(change)
{
    ...
    announce("Current velocity is " + shuttle.velocity);
}

/**
* Changes members of shuttle.seats[]
*/
function addMemberToShuttle(new_member)
{
    for(var i = 0; i < shuttle.seats.length;i++)
    {
        if(shuttle.seats[i]===null)
        {
            shuttle.seats[i] = new_member;
            break;
        }
    }
}
...
function dropoff()
{
    ...
    {
        var currPerson = shuttle.seats[i];
        if(currPerson!==null)
        {
            var dest = currPerson.substring(currPerson.lastIndexOf(" to ")+4);
            if(shuttle.distance(HOUSES[dest].lat,HOUSES[dest].lng)<=30)
            {
                ...
                if(passengersLeft===0)
                {
                    announce("You just dropped all the passengers to their houses. You rock!")
                }
            }
        }
    }
    if(!dropped)
    {
        announce("Nobody has to get off here!");
    }
}
...
{ 
    ...
    {  
        ...
        d = shuttle.distance(placemarks[i].getGeometry().getLatitude(),placemarks[i].getGeometry().getLongitude());
        if(d <= 15.0 && placemarks[i].getName().slice(-5)!=" Hall")
        {
            if(seatsAvailable == 0)
            {
                announce("Sorry, but the shuttle's full!");
                return;
            }
            else
            {
                seatsAvailable--;
                addMemberToShuttle(placemarks[i].getName());
                chart(); 
    ...
    if(!picked)
    {
        announce("There is no one here to pick!");     
    }
}
...
function populate()
{
    ...
    {
        // get the passenger's house.
        var passengerHouse = PASSENGERS[i].house;
        // pick a random building
        var building = BUILDINGS[Math.floor(Math.random() * BUILDINGS.length)];
        // this while condition makes sure that no passenger is pseudorandomly positioned at his own house.
        if(HOUSES[passengerHouse] !== undefined)
        {    
            while(building.lat==HOUSES[passengerHouse].lat && building.lng==HOUSES[passengerHouse].lng)
            {
        ...
        {
            passengersLeft--;   
            while(building.name == passengerHouse)
            {
                building = BUILDINGS[Math.floor(Math.random() * BUILDINGS.length)];   
            }
        }
        ...
        placemark.setName(PASSENGERS[i].name + " to " + passengerHouse);
